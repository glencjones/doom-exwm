...
(setq! user-full-name "Glen Jones"
       user-mail-address "glen.c.jones@gmail.com")
...
The following line
 ...
 The autostack seems to work well, but evil window movement replicated some of the window navigation
...
(if (equal (getenv "TERM") "xterm-emacs")
    (defun gcj/exwm-statup-applications ()
      ...
      (defun gcj/app-exwm/browser ()
        (interactive)
        (start-process-shell-command "Browser" "* browser *" "google-chrome-stable" ))
...
** EXWM Eperimental work

#+INCLUDE: "~/.doom.d/private_experimental.org"

** EXWM Hooks and Code to manage windo creation
...
** EXWM Hook to manage window as they are created
