...
(use-package! edwina
  ...
  ;; My origional attempt at being more sophisticated with
  ;; confirming what doom buffers I didn't want changed.
  ;; (if ( or (+popup-window-p in-buffer)
  ;;          (+popup-buffer-p in-buffer)
  ;;         ;; (+popup-mode)
  ;;          )
  ;;     (progn
  ;;       (message "Filter this buffer %s" in-buffer)
  ;;       t)
  ;;   (progn
  ;;     (message "Dont Filter %s" in-buffer)
  ;;     nil)

  ;; Remeber to use this you'll need to use the following github glencjones/edwina
  ;; In doom package.el this would require:
  ;; (package! edwina :recipe (:host github :repo "glencjones/edwina") )
  (defun doom-popup-filter (in-buffer)
    (message "[EDWINA] checking buffer [%s] [%s] [%s] [%s]" (type-of in-buffer) in-buffer (+popup-buffer-p in-buffer) +popup-mode)
